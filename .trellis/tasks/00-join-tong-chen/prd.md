...
`tong-chen` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/tong-chen/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/tong-chen/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee tong-chen` to
...
python3 ./.trellis/scripts/task.py archive 00-join-tong-chen
```
