...
- [ ] 1. Define core stock types in src/types/stocks.ts
  ...
  - _Requirements: 2, 3, 4; Non-Functional: Code Architecture_

- [ ] 2. Add ticker validation util in src/lib/validation/ticker.ts
...
- [ ] 3. Create dashboard route at /dashboard/stocks (server component)
...
- [ ] 4. Add /dashboard/stocks layout wrapper
...
- [ ] 5. Create feature module scaffolding
  - Files:
...
- [ ] 6. Implement DashboardClient orchestrator (client component)
...
- [ ] 7. Create Zustand store for dashboard state
...
- [ ] 9. Implement TradingView lightweight-charts wrapper
  - File: src/features/stock-dashboard/lib/tradingview.ts
  - Lazy-load library via dynamic import; expose `createPriceChart(container, options)`
  - Handle resize, cleanup, series setup (candles + volume)
  - Purpose: Encapsulate chart integration details
  - _Leverage: Next dynamic import; requestAnimationFrame utilities_
  - _Requirements: 2.1, 2.3, 2.4; Non-Functional: Performance

- [ ] 10. Build PriceChart component using wrapper
  - File: src/features/stock-dashboard/components/PriceChart.tsx
  - Render chart container; show skeleton and loading states; update on ticker change
  - Support zoom/pan/crosshair (via library defaults); autoscale on data load
  - Purpose: Visualize price data with smooth updates
  - _Leverage: tradingview wrapper; Tailwind classes_
  - _Requirements: 2.3, 2.4, 4.4, 4.1; Non-Functional: Performance, Usability

- [ ] 11. Create TickerInput with validation and autocomplete
  - File: src/features/stock-dashboard/components/TickerInput.tsx
  - Validate 1–5 uppercase letters; show inline errors; debounce suggestions after 2 chars
...
- [ ] 12. Implement API route: GET /api/dashboard/data
...
- [ ] 13. Implement API route: POST /api/watchlist
...
- [ ] 14. Implement WebSocket price updates endpoint
...
- [ ] 15. Wire client WebSocket manager
...
- [ ] 16. Display WatchlistCard with add/remove actions
...
- [ ] 17. Display PortfolioCard with P&L and allocation
...
- [ ] 18. Display MarketOverview (indices, trending)
...
- [ ] 20. Persist last selected ticker
...
- [ ] 21. Performance and lazy loading
  - Files: PriceChart.tsx (dynamic import), MarketOverview.tsx (suspense)
  - Lazy-load chart lib; debounce search; virtualize large lists
...
- [ ] 22. Accessibility and keyboard shortcuts
...
- [ ] 24. Optional: Set up testing (Vitest + RTL)
  - Files:
